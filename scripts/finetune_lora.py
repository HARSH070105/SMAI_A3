...
PROTOTYPES_PATH = os.path.join(ROOT_DIR, "models", "custom_monument_24_prototypes.pt") # From your previous script
...
def main():
    ...
    # Load your pre-computed optimal text vectors
    prototype_dict = torch.load(PROTOTYPES_PATH)
    class_names = list(prototype_dict.keys())

    # Stack into a fixed matrix (Shape: 24, 512). These are our "targets".
    # We set requires_grad=False because the text side is strictly frozen.
    text_prototypes = torch.stack(list(prototype_dict.values())).to(device)
    ...
    config = LoraConfig(
        r=16,                   # Rank of the update matrices (controls parameter count)
    ...
    # Use ImageFolder. Ensure it loads classes in the EXACT SAME ALPHABETICAL ORDER as your prototypes.
    train_dataset = datasets.ImageFolder(TRAIN_DIR, transform=train_transform)

    # Verify class alignment before training
    assert train_dataset.classes == class_names, "Dataset classes do not match prototype classes!"
    ...
    # ==========================================
    SAVE_PATH = os.path.join(ROOT_DIR, "models", "monument_lora_adapters")
